...
findPath
  :: HasFindPathMemo r
  => Sqlite.Database -> NodeHash -> NodeHash -> Memoize r [NodeHash]
findPath db = curry $ memoize "findPath" getFindPathMemo $ \(origin, destination) -> liftIO $ do
        ...
        if stepMapSize * sizeOf (0 :: CLong) /= stepMapSizeBytes
            then error $ "misaligned path data for " <> show destination
              ...
              Marshal.allocaArray maxLength $ \pathPtr -> do
                actualLength <- fromIntegral <$> Query.c_findPath
...
floodNodes
  :: HasFloodNodesMemo r
  => Sqlite.Database -> Int -> NodeHash -> Pattern -> Set NodeType -> Memoize r QueryResult
...
filterNodes
  ...
  => Sqlite.Database -> Int64 -> Pattern -> Memoize r QueryResult
