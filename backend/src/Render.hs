...
findPathLinks
  :: (HasUnifyComponentsMemo r, HasFindPathMemo r)
  => Sqlite.Database -> NodeMap NodeState -> [Edge] -> Memoize r [PathLink]
...
unifyComponents
  :: (HasUnifyComponentsMemo r, HasFindPathMemo r)
  => Sqlite.Database -> NodeMap NodeState -> Component -> Component -> Memoize r (Maybe PathLink)
unifyComponents db nodeStates = curry $ memoize "unifyComponents" getUnifyComponentsMemo $ \(c1, c2) -> do
